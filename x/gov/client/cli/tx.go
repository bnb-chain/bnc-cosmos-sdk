...
func GetCmdSubmitListPromotionProposal(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "submit-list-promotion-proposal",
		Short: "Submit a list promotion along with an initial deposit",
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := authtxb.NewTxBuilderFromCLI().WithCodec(cdc)
			cliCtx := context.NewCLIContext().
				WithCodec(cdc).
			...
			msg := gov.NewMsgSubmitProposal(title, string(promotionParamsBz), gov.ProposalTypeListPromotion, fromAddr, amount, votingPeriod)
