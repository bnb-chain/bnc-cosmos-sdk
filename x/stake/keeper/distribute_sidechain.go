...
// DistributeInBreathBlockWithEvents will 1) calculate rewards as Distribute does, 2) transfer commissions to all validators, and
// 3) save delegator's rewards to reward store for later distribution.
func (k Keeper) DistributeInBreathBlockWithEvents(ctx sdk.Context, sideChainId string) sdk.Events {
	// if there are left reward distribution batches in the previous day, will distribute all of them here
	// this is only a safe guard to make sure that all the previous day's rewards are distributed
	// because this case should happen in very very special case (e.g., bc maintenance for a long time), so there is no much optimization here
	var events sdk.Events
	for k.hasNextBatchRewards(ctx) {
		singleBatchEvents := k.distributeSingleBatchWithEvents(ctx, sideChainId)
...
// DistributeInBlockWithEvents will 1) actually distribute rewards to delegators, using reward store, 2) clear reward store if needed
func (k Keeper) DistributeInBlockWithEvents(ctx sdk.Context, sideChainId string) sdk.Events {
	if hasNext := k.hasNextBatchRewards(ctx); !hasNext { // already done the distribution of rewards
		return sdk.Events{}
	}

	return k.distributeSingleBatchWithEvents(ctx, sideChainId)
}
...
// distributeSingleBatchWithEvents will distribute an single batch of rewards if there is any
func (k Keeper) distributeSingleBatchWithEvents(ctx sdk.Context, sideChainId string) sdk.Events {
	// get batch rewards and validator <-> distribution address mapping
	rewards, key := k.getNextBatchRewards(ctx)
	valDistAddrs, found := k.getRewardValDistAddrs(ctx)
	if !found {
...
func transferOutRewards(k Keeper, ctx sdk.Context, rewardsMap map[string]int64, sideChainId string) (sdk.Events, error) {
	...
	for delAddr, amount := range rewardsMap {
		...
		delBscAddr, err := types.GetStakeCAoB(delAddrBytes, "Delegate")
		if err != nil {
			return sdk.Events{}, err
		}
	...
	// publish data if needed
	if ctx.IsDeliverTx() && k.PbsbServer != nil {
		event := cStake.TransferOutRewardEvent{
			ChainId:       sideChainId,
			Type:          cStake.CrossStakeTransferOutRewardType,
