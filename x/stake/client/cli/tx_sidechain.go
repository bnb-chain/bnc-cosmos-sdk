...
func GetCmdEditSideChainValidator(cdc *codec.Codec) *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		sideChainId, _, sideFeeAddr, sideVoteAddr, err := getSideChainInfo(false, false)
		if err != nil {
			return err
		}
		...
		var msg sdk.Msg
		if sideVoteAddr != nil {
			msg = stake.NewMsgEditSideChainValidatorWithVoteAddr(sideChainId, sdk.ValAddress(valAddr), description, newRate, sideFeeAddr, sideVoteAddr)
		} else {
